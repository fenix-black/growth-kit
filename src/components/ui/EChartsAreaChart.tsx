...
import { chartColorSchemes, createGradient } from '@/lib/echarts-theme';
...
export function EChartsAreaChart({
  data,
  xKey,
  series,
  height = 350,
  ...
  const option = useMemo<EChartsOption>(() => {
    ...
    const chartSeries = series.map((s, index) => {
      ...
      // Add area style if needed
      if (s.type === 'area' || s.gradient) {
        seriesConfig.areaStyle = {
          opacity: 0.2,
          color: s.gradient && index < chartColorSchemes.gradients.length
            ? createGradient(
                chartColorSchemes.gradients[index][0],
                chartColorSchemes.gradients[index][1]
              )
            : color,
        };
      }
  ...
  }, [data, xKey, series, showLegend, showGrid, colorScheme, formatter]);
