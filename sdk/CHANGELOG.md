...
### Changed
- **Modern Icon Design**: Replaced emoji icons with clean, professional SVG icons
  - Chat bubble icon when closed (filled style)
  - X icon when open (line style with rounded caps)
- **Cleaner Floating Button**: Removed credits display from floating chat button for a more standard, polished look
- Credits are now only shown in the chat panel header when opened
- More professional appearance matching industry-standard chat widgets (Intercom, Drift, Zendesk)
